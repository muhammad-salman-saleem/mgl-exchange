        ...
        <Content style={{ margin: '24px 16px 0' }}>
            <div className="site-layout-background full-height" style={{ padding: 24, minHeight: 360 }}>
                ...
                <Table dataSource={orderList} pagination={pagination} onChange={handleTableChange} loading={isLoading}>
                    <Column title="Seller" render={(text, record) => <div className="flex items-center"><img className="w-5 h-5 rounded-full mr-2" src={record.profile_pic} alt="Avatar" />{record.seller_name}</div>} key="seller_name" />
                    <Column title="Currency" dataIndex="currency" key="currency" />
                    <Column
                        title="Amount"
                        render={(text, record) =>
                            record.currency === 'USDT'
                                ? (record.amount_usdt && record.amount_usdt.toExponential())
                                : record.currency === 'MGL'
                                    ? record.amount_mgl
                                    : (record.amount_busd && record.amount_busd.toExponential())
                        }
                        key="amount"
                    />
