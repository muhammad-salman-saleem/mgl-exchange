...
class P2PService {
    ...
    static async createP2P(rawData) {
        try {
            ...
            return {response: true, message:"Success", data: rawData}
        } catch (error) {
    ...
    static async getAllP2P({ page_num, per_page }) {
        try {
            const result = await P2PModel.find();
            if (result.length === 0) {
                return { response: false, message: "There is no registered P2P.", data: null };
            }
            if (page_num && per_page) {
                const startIndex = (page_num - 1) * per_page;
                const endIndex = startIndex + per_page;
                const filteredResult = result.slice(startIndex, endIndex);
    
                return { response: true, message: "Success", data: filteredResult, totalData: result.length };
            }
    ...
    static async uploadImage(imageInfo) {
        try {
            if (imageInfo && imageInfo.file && imageInfo.file.path) {
                return { response: true, message: 'success', data: imageInfo.file.path };
            } else {
                throw new Error('Invalid imageInfo object');
            }
        } catch (error) {
            return { response: false, message: error.message, data: null };
        }
